...
4. Si el usuario de nombre completo "Spencer Low" tiene otra id. de usuario que **“spencerl”**, necesita actualizar un archivo adicional. Abra **DemoDataPreImportConfig.xml** y busque la etiqueta **userstocreateandconfigure**. Actualice la etiqueta **\<inicio de sesión\>** con el loginId (sensible a mayúsculas y minúsculas). 
...
- **\<EntitiesToEnableScheduling\>** especifica qué entidades deben ser habilitadas para reservar en la programación de Microsoft Dynamics (alias Universal Resource Scheduling).
