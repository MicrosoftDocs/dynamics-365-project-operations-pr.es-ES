...
description: Este artículo proporciona información sobre cómo gestionar y trabajar con facturas proforma basadas en proyecto.
author: rumant
ms.date: 04/05/2021
ms.topic: article
...
| **Nombre** | Pestaña **Resumen** | Se establece el nombre del contrato de proyecto de forma predeterminada. Este campo se puede editar. | 
| **Moneda** | Pestaña **Resumen** | Se establece la divisa del contrato de proyecto de forma predeterminada. Un campo de solo lectura que no puede editarse. |
...
| **Descripción** | Pestaña **Resumen** | El campo de texto que describe la factura. Este campo se puede editar. | 
| **Cobrar a** y campos relacionados | Pestaña **Resumen** | Los valores predeterminados se establecen desde el cliente del contrato del proyecto. Este campo se puede editar.  | 
| **Estado** | Pestaña **Resumen** | Establece las siguientes opciones: **Activo**, **Cerrado**, **Pagado**, y **Cancelado** y se puede editar. Los estados no admitidos para Project Operations incluyen **Cerrado** y **Cancelado**. </br> El estado se establece en **Activo** cuando se crea la factura. </br>El estado debe establecerse en **Pagado** solo después de que se confirme la factura.  | 
| **Estado de la factura de proyecto** | Pestaña **Resumen** | Establece las siguientes opciones: **Borrador**, **En revisión** y **Confirmado** y se puede editar. En los estados **Borrador** y **En revisión**, la factura se puede editar. La factura no se puede editar una vez confirmada. | 
...
| **Impuesto total** | Pestaña **Resumen** | El impuesto total de todas las líneas de factura en la factura. Un campo de solo lectura que no puede editarse. | 
...
| **Nombre** | Pestaña **General** | El nombre de la línea de la factura establecida de forma predeterminada a partir del nombre de la línea del contrato. Este campo se puede editar. |
...
| Campo | Descripción |
| --- | --- | 
| **Línea de factura** | Una referencia al **ID de línea de factura**. Este campo es de solo lectura y está bloqueado para su edición. Este enlace se puede utilizar para volver al encabezado de la factura. | 
| **Descripción** | Una descripción del detalle de línea de factura. Establecido por defecto en el campo **Comentarios internos** de **Entrada de tiempo**, y en el campo **Descripción** en **Entrada de gastos**. El campo se puede editar.| 
| **Descripción externa** | Una descripción del detalle de línea de factura. Establecido por defecto en el campo **Comentarios externos** de **Entrada de tiempo**, y en el campo **Descripción** en **Entrada de gastos**. El campo se puede editar. Esta descripción se puede utilizar para determinar qué debe estar en la factura impresa que se enviará al cliente. En Project Operations, una factura proforma no tiene todas las funciones necesarias para configurar los ajustes de impresión de una factura. | 
| **Fecha de inicio** | Es un campo de solo lectura que está establecido de forma predeterminada a partir de los datos reales del origen. |
| **Project** | Este es un campo de solo lectura que se establece de forma predeterminada desde los datos reales del origen hasta el proyecto en la línea de contrato relacionada. |  
| **Tarea** | Establecido por defecto a partir de la fuente actual. Un campo de solo lectura que no puede editarse. |
| **Categoría de transacción** | Establecido por defecto a partir de la fuente actual. Un campo de solo lectura que no puede editarse. | 
| **Rol** | Establecido por defecto a partir de la fuente actual. Un campo de solo lectura que no puede editarse. |  
| **Recurso que se puede reservar** | Establecido por defecto a partir de la fuente actual. Un campo de solo lectura que no puede editarse. | 
| **Empresa de dotación de recursos** | Establecido por defecto a partir de la fuente actual. Un campo de solo lectura que no puede editarse. | 
| **Unidad de dotación de recursos** | Establecido por defecto a partir de la fuente actual. Un campo de solo lectura que no puede editarse. | 
| **Cantidad** | Establecido por defecto a partir de la fuente actual. Un campo de solo lectura que no puede editarse. |  
| **Programación de unidad** | Para el detalle de la línea de la factura por tiempo, esto siempre se establece en tiempo y no se puede editar. Para los gastos, esto se establece de forma predeterminada a partir del gasto de origen real. Un campo de solo lectura que no puede editarse. | 
| **Unidad** | Establecido por defecto a partir de la fuente actual. Un campo de solo lectura que no puede editarse. |  
| **Precio** | Establecido por defecto a partir de la fuente actual. Un campo de solo lectura que no puede editarse. |
| **Moneda** | Establecido por defecto a partir de la fuente actual. Un campo de solo lectura que no puede editarse. | 
| **Importe** | Establecido por defecto a partir de la fuente actual. Un campo de solo lectura que no puede editarse. | 
| **Impuestos** | Establecido por defecto a partir de la fuente actual. El campo se puede editar.| 
| **Importe total** | Un campo calculado, calculado como **Importe + Impuestos**. Un campo de solo lectura que no puede editarse. | 
| **Tipo de facturación** | Establecido por defecto a partir de la fuente actual. El campo se puede editar. Al seleccionar **Cobrable**, se suma la línea al total de la línea de la factura. **Complementario** y **No imputable** lo excluirán del total de la línea de factura.| 
| **Seleccionar producto** | Establecido por defecto a partir de la fuente actual. Un campo de solo lectura que no puede editarse. |
| **Producto** | Establecido por defecto a partir de la fuente actual. Un campo de solo lectura que no puede editarse. | 
| **Nombre del producto** | Establecido por defecto a partir de la fuente actual. Un campo de solo lectura que no puede editarse. |  
| **Descripción de fuera de catálogo** | Establecido por defecto a partir de la fuente actual. Un campo de solo lectura que no puede editarse. |
| **Tipo de transacción** | Es un campo de solo lectura que está establecido de forma predeterminada a partir de los datos reales del origen a **Ventas facturadas**. |  
| **Clase de transacción** | Establecido por defecto a partir de la fuente actual. Un campo de solo lectura que no puede editarse. | 
...
| **Importe ampliado** | Establecido de forma predeterminada a partir del importe extendido del hito de origen. El campo se puede editar. | 
...
2. Abra el borrador de la factura proforma y, en la cinta **Acciones**, seleccione **Actualizar transacciones de línea de factura**.

  Los detalles de la línea de factura se crean para cualquier dato real que esté vencido y marcado como **Listo para facturar**, pero no está incluido en la factura.
